...
async function getStreakInfo(habitId) {
    const logs = await db
        ...
        .from(habitLogsTable)
        .where(and(eq(habitLogsTable.habitId, habitId), eq(habitLogsTable.completed, true)));
...
function serializeBase(h, streak, completedToday) {
    return {
        id: h.id,
        name: h.name,
...
router.patch("/habits/:id", async (req, res) => {
    ...
    const info = await getStreakInfo(row.id);
    res.json(UpdateHabitResponse.parse(serializeBase(row, info.streak, info.completedToday)));
});
...
router.post("/habits/:id/check-in", async (req, res) => {
    ...
    const [existing] = await db
        .select()
        .from(habitLogsTable)
        .where(and(eq(habitLogsTable.habitId, habit.id), eq(habitLogsTable.date, today)));
    if (existing) {
        await db
    ...
    const [refreshed] = await db
        ...
        .from(habitsTable)
        .where(eq(habitsTable.id, habit.id));
    res.json(CheckInHabitResponse.parse(serializeBase(refreshed, info.streak, info.completedToday)));
});
