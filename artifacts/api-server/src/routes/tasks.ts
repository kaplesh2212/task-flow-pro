...
router.get("/tasks", async (req: any, res: any): Promise<void> => {
  ...
  const conds = [];
  if (status && status !== "all") conds.push(eq(tasksTable.status, status));
  if (category) conds.push(eq(tasksTable.category, category));
...
router.post("/tasks/:id/toggle", async (req: any, res: any): Promise<void> => {
