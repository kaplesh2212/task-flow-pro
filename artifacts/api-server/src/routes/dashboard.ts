...
router.get("/dashboard/summary", async (_req: any, res: any): Promise<void> => {
  ...
  const completedTodaySet = new Set(
    habitLogs.filter((l) => l.date === today).map((l) => l.habitId),
  );
  const habitsCompletedToday = habits.filter((h) =>
    completedTodaySet.has(h.id),
  ).length;
  ...
  const habitRatio =
    habits.length > 0 ? habitsCompletedToday / habits.length : 0;
...
router.get("/dashboard/weekly", async (_req: any, res: any): Promise<void> => {
  ...
  const days = dates.map((d) => {
    ...
    const tasksCompleted = tasks.filter(
      (t) =>
        t.completedAt && t.completedAt.toISOString().slice(0, 10) === d,
    ).length;
  ...
  const averageScore = Math.round(
    days.reduce((a, b) => a + b.score, 0) / days.length,
  );
...
router.get("/dashboard/activity", async (_req: any, res: any): Promise<void> => {
