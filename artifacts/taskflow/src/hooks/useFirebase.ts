...
export const useFirebaseData = <T extends { id: string }>(collectionName: string) => {
  const { user } = useAuth();
...
export const useHabitLogs = (habitId: string) => {
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, `users/${user.uid}/habits/${habitId}/logs`),
      orderBy("date", "desc")
    );
  ...
  const toggleLog = async (date: string, completed: boolean) => {
    if (!user) return;
    ...
    if (!isFirebaseConfigured) {
      ...
      let logsList: any[] = [];
      if (stored) {
        try { logsList = JSON.parse(stored); } catch(e){}
      }
      const existingIdx = logsList.findIndex((l: any) => l.date === date);
      if (existingIdx !== -1) {
